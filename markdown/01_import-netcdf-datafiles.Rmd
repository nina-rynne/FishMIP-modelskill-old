...
watson_obs_sum_LME <- watson_filtered %>%
  group_by(Year, LME)%>% 
  summarise(rec_obs = sum(Reported+IUU+Discards)) %>%
  mutate("Source" = "WAT")

watson_obs_sum_global <- watson_filtered %>%
  group_by(Year)%>%
  summarise(rec_obs = sum(Reported+IUU+Discards)) %>%
  mutate("LME" = "0", "Source" = "WAT") %>%
...
saup_obs_sum_global <- saup_filtered %>%
  group_by(Year) %>%
  summarise(rec_obs = sum(tonnes))%>%
  mutate("LME" = "0", "Source" = "SAU") %>%
  relocate(LME, .after = Year)
...
# -------------------------------------------------------
write.csv(watson_obs_sum_global, here("output/reconstructedcatch_WAT_global.csv"), row.names = FALSE)
write.csv(saup_obs_sum_global, here("output/reconstructedcatch_SAU_global.csv"), row.names = FALSE)
write.csv(watson_obs_sum_LME, here("output/reconstructedcatch_WAT_LME.csv"), row.names = FALSE)
