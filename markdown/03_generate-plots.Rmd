...
p1 <- ggplot()+
  geom_line(data = modeldatap1, aes(x = Year, y = proj_tc_norm, group = Model, color = Model), linewidth = 1.05)+
  geom_line(data = obs_tc_byYear_global_norm, aes(x = Year, y = rec_obs_norm, group = Source, color = Source), linewidth = 1.25)+
  xlab("\nYear")+
  ylab("Total Catch \n(million tonnes)")+
  my_theme+
...
p2 <- ggplot()+
  geom_point(data = modeldatap2, aes(x = rec_obs_norm, y = proj_tc_norm, group = ESMs, color = ESMs), show.legend = FALSE)+
  my_theme+
...
p1 <- ggplot()+
  geom_line(data = modeldatap1, aes(x = Year, y = proj_tc_norm, group = Model, color = Model), linewidth = 1.05)+
  geom_line(data = obsdatap1, aes(x = Year, y = rec_obs_norm, group = Source, color = Source), linewidth = 1.25)+
  xlab("\nYear")+
  ylab("Total Catch \n(million tonnes)")+
  my_theme+
...
ri_data_boats <- stats_byLME_byModel %>%
  filter(MEMs == 'boats')
...
ri_data_ecoocean <- stats_byLME_byModel %>%
  filter(MEMs == 'ecoocean')
