...
getLMEStatsRAW <- function(mem_name, esm_name) {
...
getGlobalStatsRAW <- function(mem_name, esm_name) {
...
for(i in 1:length(mems)){
  for(j in 1:length(esms)){
    ...
    LMEstats_RAW_return <- getLMEStatsRAW(mem_name = mems[i],
                                   esm_name = esms[j])
    
    # decide how to handle data returned from function
    
    if(!exists("stats_byLME_byModel_RAW")){
        stats_byLME_byModel_RAW <- LMEstats_RAW_return
      }else{
        stats_byLME_byModel_RAW <- rbind(stats_byLME_byModel_RAW, LMEstats_RAW_return)
      } # end if/else loop
...
```{r call_global_stats_RAW_function}
...
for(i in 1:length(mems)){
  for(j in 1:length(esms)){
    ...
    globalstats_RAW_return <- getGlobalStatsRAW(mem_name = mems[i],
                                  esm_name = esms[j])
    
    # decide how to handle data returned from function
    
    if(!exists("stats_global_byModel_RAW")){
        stats_global_byModel_RAW <- globalstats_RAW_return
      }else{
        stats_global_byModel_RAW <- rbind(stats_global_byModel_RAW, globalstats_RAW_return)
      } # end if/else loop
...
# statistical test results by LME
saveRDS(object = stats_byLME_byModel_RAW, file = here("output/stats_byLME_byModel_RAW.rds"))

# statistical test results, globally
saveRDS(object = stats_global_byModel_RAW, file = here("output/stats_global_byModel_RAW.rds"))
