...
modeloutputs_byYear <- modeloutputs_byYear %>% dplyr::group_by(MEMs, ESMs, Element, LME) %>%
                                dplyr::mutate(start_val = mean(Value[between(Year, min(Year),min(Year)+5)]),
                                              Value = 100*(Value - start_val)/start_val)



#example <- modeloutputs_byYear %>% dplyr::filter(LME == 0 & MEMs == "boats" & ESMs == "gfdl-esm2m", Element=="tc_tonnes_year")

#plot.new()
#ggplot(data = example) + geom_line(aes(x=Year, y = delta_val))
...
# create table with total catch only

mod_tc_byYear <- modeloutputs_byYear %>%
  filter(Element == "tc_tonnes_year") %>%
  mutate("Model" = paste(MEMs, ESMs)) 

tc_byYear_withObs_global <- mod_tc_byYear %>%
  filter(Element == "tc_tonnes_year") %>%
  filter(LME == "0") %>%
  left_join(reconstructedWAT_byYear_global, by = join_by(LME, Year)) %>%
  mutate("Model" = paste(MEMs, ESMs)) %>% 
  dplyr::group_by(Model) %>% 
  dplyr::mutate(obs_start = mean(rec_obs[between(Year, min(Year), min(Year) + 5)]),
                rec_obs = 100*(rec_obs - obs_start)/obs_start)

tc_byYear_withObs_LME <- mod_tc_byYear %>%
  filter(Element == "tc_tonnes_year" & LME != 0) %>%
  left_join(reconstructedWAT_byYear_LME, by = join_by(LME, Year)) %>%
  mutate("Model" = paste(MEMs, ESMs)) %>% dplyr::group_by(LME, Model, Source) %>%
  dplyr::mutate(obs_start = mean(rec_obs[between(Year, min(Year), min(Year) + 4)]),
                rec_obs = 100*(rec_obs - obs_start)/obs_start)
...
obs_tc_byYear_global <- rbind(reconstructedSAU_byYear_global, reconstructedWAT_byYear_global) %>%
  dplyr::group_by(LME, Year, Source) %>%
  dplyr::mutate(obs_start = mean(rec_obs[between(Year, min(Year), min(Year) + 4)]),
                rec_obs = 100*(rec_obs - obs_start)/obs_start)
